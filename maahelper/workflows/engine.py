...
class WorkflowEngine:
        ...
        with progress:
            ...
            while len(completed_steps) + len(failed_steps) < len(workflow.steps):
                ...
                for step in ready_steps:
                    task = self._execute_step(step, context, workflow.id, step_tasks[step.id], progress)
                    tasks.append(task)
    ...
    async def resume_workflow(self, workflow_id: str) -> bool:
        ...
        if workflow_id not in self.active_workflows:
            ...
            if state:
                ...
                workflow = self._hydrate_workflow_definition(workflow_data)
                self.active_workflows[workflow_id] = workflow
            else:
                return False
    ...
    def _hydrate_workflow_definition(self, data: Dict[str, Any]) -> WorkflowDefinition:
        """Recreate WorkflowDefinition and WorkflowStep objects from plain data"""
        # Rebuild steps
        steps = []
